...
<div class="card" style="width: 18rem; hover">
    @foreach ($images as $image)
    @if($image->product_id == $item->id)
    <img class="card-img-top hover" src="{{ asset('storage/' . $image->url) }}" alt="image" style="object-fit: cover; width: 100%; height: 300px;"/>
    @break {{-- Exit the loop after the first image --}}
    @endif
    @endforeach
    <div class="card-body"  >
        ...
        <p class="card-text">{{ strlen($item->summary) > 60 ? substr($item->summary, 0, 60) . ' ...' : $item->summary }}</p>
        <a href="#" class="btn btn-outline-dark">More</a>
    </div>
</div>
